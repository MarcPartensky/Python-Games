...
class Chess:
    ...
    def loadThemes(self):
        ...
        self.selecters_colors_classic = [
            [BEIGE, WHITE],
            [BEIGE, WHITE],
            [BEIGE, WHITE],
            [BEIGE, WHITE],
        ]
        self.selecters_colors_dark = [
            [LIGHTGREY, BLACK],
            [WHITE, BLACK],
            [HALFGREY, BLACK],
            [DARKGREY, BLACK],
        ]
        self.selecters_colors_light = [
            [GREY, WHITE],
            [BLACK, WHITE],
            [WHITE, BLACK],
            [GREY, WHITE],
        ]
        self.selecters_colors_random = [
            [randomColor(), randomColor()] for i in range(4)
        ]

        self.theme_classic = [
            self.pieces_colors_classic,
            self.grid_colors_classic,
            self.selecters_colors_classic,
        ]
        self.theme_dark = (
            self.piece_colors_dark,
            self.grid_colors_dark,
            self.selecters_colors_dark,
        )
        self.theme_light = (
            self.piece_colors_light,
            self.grid_colors_light,
            self.selecters_colors_light,
        )
        self.theme_random = (
            self.piece_colors_random,
            self.grid_colors_random,
            self.selecters_colors_random,
        )
    ...
    def play(self):
        ...
        if len(self.board.getSideMoves(self.players[turn].side)) == 0:
            ...
            board = (
                self.board
            )  # supposed to be giving the player a copy of the board, but is actually giving the actual one
            ...
            if self.players[turn].hasChosen:
                ...
                if (
                    entity and move in moves
                ):  # Testing if player's choice is possible before changing the true board
                    self.board.move(entity, position, move)
                    self.board.updateHistoric()
                    print(
                        self.board.getPresentation(
                            board.state, (entity, position, move)
                        )
                    )
