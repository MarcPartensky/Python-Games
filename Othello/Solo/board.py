...
class Board:
    ...
    def check(self):
        moves=self.getMoves(self.grid,0,None)
        #print(moves)
        if len(moves)==0:
            moves=self.getMoves(self.grid,1,None)
            #print(moves)
            if len(moves)==0:
                self.won=True
    ...
    def getMoves(self,grid,player_side,window):
        #print(player_side)
        enemy_side=(player_side+1)%2
        positions=self.getPieces(grid,enemy_side)
        #for position in positions:
        #    self.debugMove(position,window,PURPLE)
        positions=self.getAround(grid,positions)
        #for position in positions:
        #    self.debugMove(position,window,YELLOW)
        moves=[]
        positions=list(set(positions))
        for position in positions:
            if self.possibleMove(grid,position,player_side,window):
                moves.append(position)
        return list(set(moves))

    def getPieces(self,grid,side):
        positions=[]
        #print(side)
        side+=1
        sx,sy=self.size
    ...
    def getAround(self,grid,positions):
        environment=[]
    ...
    def conquer(self,grid,position,p_side):
        px,py=position
        p_side+=1
        e_side=p_side%2+1
        sx,sy=self.size
    ...
    def possibleMove(self,grid,position,p_side,window):
        self.debugMove(position,window,YELLOW)
        px,py=position
        p_side+=1
        e_side=p_side%2+1
