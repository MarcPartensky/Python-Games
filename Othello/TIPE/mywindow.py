...
class Window:
    ...
    def draw(self):
        ...
        radius=2
        wavelength=380
        color=self.reverseColor(self.couleur_de_fond)
        form=0
        taille=[10,10]
        width=0
        while self.open:
            ...
            position=self.point()
            if click:
                self.trace(position,taille,self.wavelengthToRGB(wavelength),radius,form,width)
            else:
                self.hand[0].points=[]
                #print(taille)
            keys=pygame.key.get_pressed()
            ...
            if keys[K_RIGHT] and wavelength<780:
                wavelength+=1
            if keys[K_q] and taille[0]>0:
                taille[0]-=1
            if keys[K_w]:
                taille[0]+=1
            if keys[K_e] and taille[1]>0:
                taille[1]-=1
            if keys[K_r]:
                taille[1]+=1
            if keys[K_s]:
