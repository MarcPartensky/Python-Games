...
try:
    DIRECTORY="/Users/olivierpartensky/Programs/Python/Games/Snake/"
    os.chdir(DIRECTORY)
finally:
    pass
...
class Map:
    ...
    def oldshow(self,window):
        ...
        for x in range(mx):
            for y in range(my):
                window.draw(self.color_case,(cx*x+1,cy*y+1),(cx-1,cy-1))
...
class Snake:
    ...
    def spawn(self,map):
        ...
        mx,my=map.size
        self.head=[random.randint(0,mx-1-self.size),random.randint(0,my-1-self.size)]
        self.body=[self.head]
        x,y=self.head
        r=self.direction
...
class Food:
    ...
    def spawn(self,map):
        mx,my=map.size
        self.coordonnates=(random.randint(0,mx-1),random.randint(0,my-1))
