...
class Motion:
    ...
    def __init__(self, *vectors):
        ...
        if len(self.vectors) >= 1:
            self.position.color = mycolors.GREEN
        if len(self.vectors) >= 2:
            self.velocity.color = mycolors.BLUE
        if len(self.vectors) >= 3:
            self.acceleration.color = mycolors.RED
...
class Moment(Motion):
    ...
    def __init__(self, *vectors, n=2, d=1):
        ...
        self.vectors = list(vectors)
        if len(self.vectors) >= 1:
            self.position.color = mycolors.PURPLE
        if len(self.vectors) >= 2:
            self.velocity.color = mycolors.ORANGE
        if len(self.vectors) >= 3:
            self.acceleration.color = mycolors.YELLOW
