...
class Body(Physics):
    ...
    @classmethod
    def random(cls, corners=[-1, -1, 1, 1], n=5, nm=2):
        """Create a random body."""
        anatomy = Form.random(corners, n)
        anatomy.recenter()
        motions=[]
        if nm>=1: motions.append(Motion.random())
        if nm>=2: motions.append(Moment.random())
        if nm>=3: motions.extend([Motion.random() for i in range(nm-2)])
    ...
    def createFromRandomMotions(cls,anatomy,n=2):
        ...
        motions=[]
        if n>=1: motions.append(Motion.random())
        if n>=2: motions.append(Moment.random())
        if n>=3: motions.extend([Motion.random() for i in range(n-2)])
        return cls(anatomy,*motions)
    ...
    def showMoment(self, context):
        ...
        p=self.motion.velocity(Point(*self.position))
        v=(self.motion.velocity/self.motion.velocity.norm)
        v.rotate(-math.pi/2)
        v.show(context,p)
        self.moment.acceleration.show(context,p)
