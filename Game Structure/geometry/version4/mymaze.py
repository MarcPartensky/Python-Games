...
class BasicEntity(Case):
    """Defines a movable entity with a direction, a position, a size and a show method."""
...
class Maze:
    ...
    def __init__(self,position=[0,0],size=[20,20],fill=True):
        """Create a maze using size."""
        self.position=position
        self.fill=fill
        self.matrix=np.array(Maze.conversion(Maze.convert(Maze.eller(Maze.make_empty_grid(*size))),['X',' '],[1,0]))
