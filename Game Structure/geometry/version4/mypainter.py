...
class Paint:
    ...
    def random(position=[0,0],size=[10,10]):
    ...
    def getForm(self):
        ...
        xmin,ymin,xmax,ymax=self.getCorners()
        ps=[Point(xmin,ymin),Point(xmax,ymin),Point(xmax,ymax),Point(xmin,ymax)]
        return Form(ps)
...
class Board(Paint):
    ...
    def generate(self):
        ...
        x,y=self.position
        sx,sy=self.size
        self.cases=[Case([x,y-sy//2],color=mycolors.random()) for y in range(sy)]
